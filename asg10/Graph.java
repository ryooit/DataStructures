	 ...
	 while ( ! fringe.isEmpty() ) {
		 ...
		 if ( v.equals(goal) )
			 ...
			 for ( Edge e : v.edges ) { 
				 ...
				 if ( newcost < e.target.cost ) { 
					 e.target.cost = newcost;
		             e.target.estimate = v.cost + e.cost + h.fn(e.target,goal);
		             e.target.parent = v;
		             fringe.add(e.target);  
		             } 
				 }
...
public int edgecost( Vertex start, Vertex goal) {
	int cost = 0;
	Vertex v = vertices.get(goal.contents);
	ArrayList<Edge> edges = v.edges;
	for (Edge edge : edges) {
		if ( (edge.target).contents.equals(start.contents) )
			cost = edge.cost;
	}
	return cost;
}
...
public int pathcost (Vertex v) {
	Cons path = pathto(v.contents);
	int cost = 0;
	while (Cons.rest(path) != null) {
		cost += edgecost(vertices.get(Cons.first(path)),vertices.get(Cons.second(path)));
		path = Cons.rest(path);
	}
	return cost;
}
...
public Cons pathto(String city) {
	Cons path = null;
	Vertex v = vertices.get(city);
	while ( v.parent() != null ) {
		path = Cons.append(Cons.list(v.contents),path);
		v = v.parent();
	}
	path = Cons.append(Cons.list(v.contents),path);
	return path;
