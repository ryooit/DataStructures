...
// empty buffers into a music string
    ...
    public static Cons solve(Cons e, String v) {
    	if ( lhs(e).equals(v) )
    		...
    		Cons algebra = algpats;
    		while ( algebra != null ) {
    			Object result = transform((Cons)first(algebra),e);
    			if ( result != null ) {
    				Cons x = solve((Cons)result,v);
    				if ( x != null )
    					return x;
    			}
    			algebra = rest(algebra);
    		}
    		return null;
    	}
    	else
    ...
    public static int conshash(Object e) {
    	if ( e == null )
    		...
    		return first((Cons)e).hashCode()*17 ^ conshash((Object)rest((Cons)e))*127;
    }
    ...
    public static int totaltime(Cons action) {
    	if ( first(action).equals("repeat") )
    		...
    		Cons newlst = null;
    		Cons lst = rest(action);
    		while ( lst != null ) {
    			newlst = cons(totaltime((Cons)first(lst)),newlst);
    			lst = rest(lst);
    		}
    		return sumlistr(newlst);
    	}
    ...
    public static void execute(PriorityQueue pq, Cons act, int time) {
        ...
        if ( command.equals("tambourine") ) {
        	int newtime = 0;
        	double rand = (int)(Integer)second(act)*random.nextDouble();
        	emit(11, time, 2);
        	while (rand >= 2 && newtime <= (int)(Integer)second(act))
        	{
        		while (rand >= 20)
        		{
        			rand = (int)(Integer)second(act)*random.nextDouble();
        		}
        		newtime += rand;
        		if (newtime > (int)(Integer)second(act))
        		{
        			return;
        		}
        		addevent(pq,list("tambourine",2),newtime);
        		rand = (int)(Integer)second(act)*random.nextDouble();
        	}
        	return;
        }
        ...
        if ( command.equals("kaboom") ) {
        	int t = (int)(Integer)second(act);
            addevent(pq,list("seq",list("rest",t/4),list("boom",t/4),list("boom",t/2)),time);
            return; }
        ...
        if (command.equals("seq"))
        {
        	...
        	while (x != null)
        	{
        		addevent(pq,(Cons)first(x),newtime);
        		newtime += totaltime((Cons)first(x));
        		x = rest(x);
        	}
        	return;
        }
        ...
        {
        	int newtime = time;
        	int x = (int)second(act);
        	while (x > 0)
        	{
        		addevent(pq,(Cons)third(act),newtime);
        		newtime += totaltime((Cons)third(act));
        		x--;
        	}
        	return;
        }
    ...
    public static Cons round( Cons melody ) {
    	int d = totaltime(melody);
    	Cons x = list("sync",melody,list("seq",list("rest",d/4),melody),list("seq",list("rest",d/2),melody));
    	return (Cons) subst( Integer.valueOf(0),"?i", x);
    }
